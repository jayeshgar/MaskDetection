"""Utility functions for mask_detector module."""
...
def collate_double(batch):
    ...
    """
    #print("batch = ",batch)
    x = [torch.unsqueeze(sample[0],0) for sample in batch]
    y = [sample[1] for sample in batch]
    x = torch.cat(x)
    #print("x afterwards shape = ",x.shape)
    #x_name = [sample[1]["image_id"] for sample in batch]
    #y_name = [sample[1]["image_id"] for sample in batch]
    return x, y